...
IDLE = Tk()
DesiredFont = TkFont.Font(family="Courier New", size=13)
    ...
    try:
        while i < len(toks):
            ...
            if toks[i] == "START":
                ...
                error_line_information = "Ask Computer User" + " " + "\"" + toks[i + 1][7:] + "\"" + " " + "and store the answer in" + " " + "Variable[" + toks[i + 3][4:] + "]"
                string = toks[i+1][7:]
                varname = toks[i+3]
...
file_name_bar = Text(width = 155, height = 1, font=DesiredFont)
file_name_bar.insert("1.0", f"Name of Current File: {file_name_extracter(file_path)}")
file_name_bar.pack()
editor_header = Text(width = 155, height = 1, font=DesiredFont)
editor_header.insert("1.0", ">>> Program Editor <<<")
editor_header.pack()
editor = Text(width = 155, height = 27, font=DesiredFont, wrap="none")
editor.pack()
input_box_header = Text(width = 155, height = 1, font=DesiredFont)
input_box_header.insert("1.0", ">>> Input Box <<<")
input_box_header.pack()
input_box = Text(width = 155, height = 1, font=DesiredFont)
input_box.pack()
code_output_header = Text(width = 155, height = 1, font=DesiredFont)
code_output_header.insert("1.0", ">>> Program's Output <<<")
code_output_header.pack()
code_output = Text(width = 155, height = 16, font=DesiredFont, wrap="none")
code_output.pack()
IDLE.mainloop()
